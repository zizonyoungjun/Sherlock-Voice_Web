...
const PhishingCategory: React.FC<PhishingCategoryProps> = ({ keywords }) => {
  ...
  useEffect(() => {
    if (keywords.includes("대출")) {
      ...
      setInstructionText("대출 사기형이란, 낮은 금리로 대출을 해준다며 선입금이나 수수료를 요구하는 유형입니다. 실제로는 대출이 이루어지지 않고, 선입금만 가로채는 방식으로 사용자에게 금전적인 피해를 줍니다. 아래 메뉴얼을 따라주세요!");
    } else if (keywords.includes("검찰청")) {
      setLottieAnimation(governmentLottie);
      setInstructionTitle("분석한 결과,\n'기관 사칭형'이\n의심됩니다!");
      setInstructionText("기관 사칭형이란, 경찰서, 검찰청, 금융감독원, 은행 등의 기관을 사칭하여 사용자의 계좌 정보나 개인 정보를 요구하는 유형입니다. 주로 범죄 연루 혐의 등을 빌미로 협박하여 사용자의 정보를 탈취하려고 합니다. 아래 메뉴얼을 따라주세요!");
    } else if (keywords.includes("복권")) {
      setLottieAnimation(lotteryLottie);
      setInstructionTitle("분석한 결과,\n'복권 당첨형'이\n의심됩니다!");
      setInstructionText("복권 당첨형이란, 복권에 당첨되었다거나 큰 금액의 상금을 받았다는 거짓 정보를 주고, 상금을 받기 위해 세금이나 수수료를 먼저 납부하라고 요구하는 유형입니다. 이는 실제로 존재하지 않는 상금을 빌미로 사용자의 돈을 가로채려는 수법입니다. 아래 메뉴얼을 따라주세요!");
    } else if (keywords.includes("상품권")) {
      setLottieAnimation(giftCardLottie);
      setInstructionTitle("분석한 결과,\n'상품권 사기형'이\n의심됩니다!");
      setInstructionText("상품권 사기형이란, 상품권이나 기프트카드를 구입하도록 유도한 뒤, 그 코드를 요구하는 유형입니다. 주로 이메일이나 문자 메시지를 통해 접근하여 사용자의 상품권 정보를 탈취하려고 합니다. 아래 메뉴얼을 따라주세요!");
    }
  }, [keywords]);
