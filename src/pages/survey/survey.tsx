...
const Survey: React.FC = () => {
  ...
  const handleYes = () => {
    ...
    moveToNextQuestion(newYesCount, noCount);
  };

  const handleNo = () => {
    const newNoCount = noCount + 1;
    setNoCount(newNoCount);
    moveToNextQuestion(yesCount, newNoCount);
  };

  const moveToNextQuestion = (updatedYesCount: number, updatedNoCount: number) => {
    if (currentQuestionIndex < questions.length - 1) {
      setCurrentQuestionIndex(currentQuestionIndex + 1);
    } else {
      navigate(`/SurveyResult?yes=${updatedYesCount}&no=${updatedNoCount}`);
    }
  };
